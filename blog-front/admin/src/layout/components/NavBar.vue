...
import { resetMenuRouteLoaded } from "../../assets/js/menu";
export default {
  created() {
    //替换面包屑导航
  ...
  methods: {
    ...
    handleCommand(command) {
      ...
      if (command == "logout") {
        // 先调用后端 /logout 让 sa-token 主动清 Redis 中的 token,
        // 再在 finally 里清本地状态:即便后端 5xx/超时也能正常跳回登录页,避免卡死。
        // 注意 token 清理放在 .finally 里,确保 /logout 请求自身仍能带上 Authorization Header。
        this.axios.post("/api/logout").finally(() => {
