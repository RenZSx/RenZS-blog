      ...
      >
        <template slot-scope="scope">
          ...
          <i
            ...
            class="el-icon-star-on article-status-icon"
            style="color: #ff9900;"
          />
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          ...
          >
            <el-option label="公开" :value="1">
              <i class="el-icon-view" style="margin-right: 5px;"></i>公开
            </el-option>
            <el-option label="私密" :value="2">
              <i class="el-icon-lock" style="margin-right: 5px;"></i>私密
            </el-option>
            <el-option label="草稿" :value="3">
              <i class="el-icon-document" style="margin-right: 5px;"></i>草稿
            </el-option>
            <el-option label="推荐" :value="4">
              <i
                class="el-icon-star-on"
                style="margin-right: 5px; color: #ff9900;"
              ></i
              >推荐
            </el-option>
          </el-select>
...
export default {
  ...
  methods: {
    ...
    changeArticleStatus(article) {
      ...
      // 如果要设置为推荐，先检查全站是否已有推荐文章
      if (article.status === 4) {
        this.listRecommendArticles()
          .then(existingRecommendList => {
            const otherRecommendList = existingRecommendList.filter(
              item => item.id !== article.id
            );
            if (otherRecommendList.length) {
              const articleTitles = otherRecommendList
                .map(item => `《${item.articleTitle}》`)
                .join("、");
              this.$confirm(
                `已存在推荐文章：${articleTitles}，是否将原来的推荐文章改为公开，并将当前文章《${article.articleTitle}》设为推荐？`,
                "提示",
                {
                ...
                .then(() => {
                  this.updateArticleStatus(article, statusMap, true);
                })
                .catch(() => {
                  this.listArticles();
                });
            ...
            this.updateArticleStatus(article, statusMap);
          })
          .catch(error => {
            this.$notify.error({
              title: "失败",
    ...
    listRecommendArticles() {
      return this.axios
        ...
        .then(({ data }) => {
          ...
          throw new Error(data.message || "推荐文章检测失败");
        });
    },
    updateArticleStatus(article, statusMap, replacedRecommend = false) {
      // 先获取完整的文章数据
      this.axios
        ...
        .then(({ data }) => {
          if (data.flag) {
            // 使用完整的文章数据，只修改status
            const articleVO = data.data;
            articleVO.status = article.status;

            // 保存文章
            return this.axios.post("/api/admin/articles", articleVO);
          } else {
        ...
        .then(({ data }) => {
          if (data.flag) {
            this.$notify.success({
              title: "成功",
              message: replacedRecommend
                ? "已将原推荐文章改为公开，并将当前文章设置为推荐"
                : `文章已设置为${statusMap[article.status]}`
            });
            this.listArticles();
          } else {
            this.$notify.error({
        ...
        .catch(error => {
          this.$notify.error({
            title: "失败",
            message:
              error.response?.data?.message || error.message || "状态修改失败"
          });
          this.listArticles();
        });
    },
